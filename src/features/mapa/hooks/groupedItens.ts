...
export const groupedData = ({ enrichedData, isFifo, fifoRanges }: Props) => _.groupBy(enrichedData, (item: any) => {
    ...
    if (segregatedClients.length > 0) {
      if (isFifo) {
        if (segregatedClients.find(client => client.includes(item?.customerCode)) && fifoRanges?.includes(item?.faixaProduto?.faixa)) {
          return `FIFO-${segregatedClients.find(client => client.includes(item?.customerCode))}[${item?.transport}] - ${item?.customerName} - ${item?.warehouseProduct?.line} - ${item?.warehouseProduct?.empresa}`;
        }
      }
      if (segregatedClients.find(client => client.includes(item?.customerCode))) {
        return `${segregatedClients.find(client => client.includes(item?.customerCode))}[${item?.transport}] - ${item?.customerName} - ${item?.warehouseProduct?.line} - ${item?.warehouseProduct?.empresa}`;
      }
    }
    //Agrupar transporte
    if (isFifo) {
  ...
  if (clientGroups.length > 0) {
    if (isFifo) {
      if (fifoRanges?.includes(item?.faixaProduto?.faixa)) {
        if (clientGroups.find(group => group.items.includes(item?.customerCode))) {
