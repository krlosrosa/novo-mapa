...
export default function UploadProduct() {
  ...
  return (
    <div className="space-y-4">
          ...
          <div className="flex flex-col items-center gap-2 p-3 bg-green-50 border border-green-200 rounded-lg">
            <div className="flex items-center gap-2">
              ...
              <Badge variant="secondary" className="ml-auto">
                {dataWarehouseProduct.length} registros
              </Badge>
            </div>
            <div className="flex w-full justify-center mt-2">
              <Button
                ...
                size="sm"
                onClick={() => setDataWarehouseProduct([])}
                type="button"
                aria-label="Trocar arquivo de produto"
              >
                Trocar arquivo
              </Button>
            </div>
      ...
      {/* Upload Area */}
      <div
        className={`border-2 border-dashed rounded-lg p-6 text-center transition-colors ${dataWarehouseProduct && dataWarehouseProduct.length > 0
      ...
      >
        <div className="space-y-3">
          ...
          <input
            ...
            tabIndex={-1}
          />
          {(!dataWarehouseProduct || dataWarehouseProduct.length === 0) && !isLoading && (
            <>
              <Button
                variant="outline"
                size="sm"
                ...
                className="mt-2"
                type="button"
                aria-label="Selecionar arquivo de produto"
              >
                <FileText className="h-4 w-4 mr-2" />
                Selecionar Arquivo
              </Button>
              <label htmlFor="product-upload-fallback" className="sr-only">Selecionar Arquivo</label>
              <input
                id="product-upload-fallback"
                type="file"
                accept=".xlsx, .xls"
                onChange={handleFileChange}
                style={{ display: 'none' }}
      ...
      {dataWarehouseProduct && dataWarehouseProduct.length > 0 && (
        <div className="text-xs text-muted-foreground text-center">
          ...
          <p>Dados de produtos processados</p>
        </div>
      )}
    </div>
  );
